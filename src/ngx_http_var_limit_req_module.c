...
typedef struct {
    ...
    /* integer value, 1 corresponds to 0.001 r/s */
    ngx_uint_t                   burst;
} ngx_http_var_limit_req_status_item_t;
...
static ngx_int_t ngx_http_var_limit_req_monitor_handler(ngx_http_request_t *r);
...
static ngx_command_t  ngx_http_var_limit_req_commands[] = {
    ...
    { ngx_string("var_limit_req_monitor"),
      NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
      ngx_http_var_limit_req_monitor,
      NGX_HTTP_LOC_CONF_OFFSET,
      0,
      NULL },
...
ngx_http_var_limit_req_top_handler(ngx_http_request_t *r)
{
    ...
    rc = ngx_http_var_limit_req_status_build_response_body(r, &items, &out);

    if (rc != NGX_OK) {
        return rc;
    }

    rc = ngx_http_var_limit_req_add_uint_header(r, &num_all_keys_name,
                                                num_all_keys);
    if (rc != NGX_OK) {
        return rc;
    }

    rc = ngx_http_var_limit_req_add_uint_header(r, &top_n_name, items.nelts);
    if (rc != NGX_OK) {
        return rc;
    }
...
static ngx_int_t
ngx_http_var_limit_req_add_uint_header(ngx_http_request_t *r,
    const ngx_str_t *lowcase_key, ngx_uint_t value)
{
    ngx_table_elt_t                *h;
    ...
    ngx_rbtree_t *rbtree, ngx_array_t *items, ngx_uint_t *num_all_keys)
{
    ...
    *num_all_keys = 0;
    ...
    rc = ngx_array_init(items, r->pool, top_n,
                        sizeof(ngx_http_var_limit_req_status_item_t));
    if (rc != NGX_OK) {
        return NGX_HTTP_INTERNAL_SERVER_ERROR;
    }
         ...
         node = ngx_rbtree_next(rbtree, node))
    {
        ...
        ngx_http_var_limit_req_set_status_item(&tmp_item, lcn, now);
...
static void
ngx_http_var_limit_req_set_status_item(
    ngx_http_var_limit_req_status_item_t *item,
    ngx_http_var_limit_req_node_t *lcn, ngx_msec_t now)
{
    ngx_msec_int_t  ms;
    ...
    item->adjusted_excess = ngx_http_var_limit_req_adjust_excess(
    ...
    const ngx_http_var_limit_req_status_item_t *item)
{
    ngx_uint_t                                   min_index, one_past_max_index,
                                                 index;
    ngx_int_t                                    res;
    const ngx_http_var_limit_req_status_item_t  *item2;
...
static ngx_int_t
ngx_http_var_limit_req_status_build_response_body(ngx_http_request_t *r,
    ngx_array_t *items, ngx_chain_t *out)
{
    ngx_uint_t                             i;
    ngx_http_var_limit_req_status_item_t  *item, *items_ptr;
    size_t                                 buf_size = 0;
    ngx_buf_t                             *b;
...
{
    const ngx_http_var_limit_req_status_item_t  *item_a, *item_b;
    size_t                                       n;
    ngx_int_t                                    rc;
...
static ngx_uint_t
ngx_http_var_limit_req_count_keys(ngx_str_t *comma_separated_value)
{
    u_char      *p, *start, *last;
    ngx_uint_t   n = 0;
...
static ngx_int_t
ngx_http_var_limit_req_next_key(ngx_str_t *comma_separated_value,
    ngx_str_t *key)
{
    u_char  *p, *start, *last;
    ...
    ngx_rbtree_t *rbtree, ngx_array_t *items)
{
    ...
    lrcf = ngx_http_get_module_loc_conf(r, ngx_http_var_limit_req_module);
    ngx_str_null(&keys);
    arg_key_val = ngx_http_get_flushed_variable(r, lrcf->arg_key_var_index);
    if (arg_key_val->valid) {
        keys.data = arg_key_val->data;
        keys.len = arg_key_val->len;
        key_count = ngx_http_var_limit_req_count_keys(&keys);
    }
    ...
    ngx_str_null(&key);
    while (ngx_http_var_limit_req_next_key(&keys, &key)) {
        lr = ngx_http_var_limit_req_do_lookup(rbtree, &key);
        if (lr == NULL) {
            continue;
        }
         ...
         */
        item = ngx_array_push(items);
        ngx_http_var_limit_req_set_status_item(item, lr, now);
    }
...
static ngx_int_t
ngx_http_var_limit_req_monitor_handler(ngx_http_request_t *r)
{
    ngx_http_var_limit_req_conf_t  *lrcf;
    ngx_shm_zone_t                 *shm_zone;
    ngx_http_var_limit_req_ctx_t   *ctx;
    ...
    void *conf)
{
    ...
    lrcf->arg_key_var_index = ngx_http_get_variable_index(cf, &arg_key_name);
